...
- 红蓝双方各 **随机 1 个兵种**（均从该主题池抽取），数量用 **LCM 平衡资源**——与指定兵种对决（2 个指定兵）逻辑一致，不是单挑 1v1
...
- `@机器人 斗蛐蛐 兵种A 兵种B [预算]`
- `@机器人 斗蛐蛐 兵种A 数量 兵种B 数量`（固定数量，每方 1~1000）
- 未指定数量时继续使用 LCM 资源平衡；指定后严格使用玩家数量，时代改良与通用科技不改变数量
- 详见 `lineup.py :: generate_custom_lineup`
...
| 维度 | A 押注 | B 单挑 | C 乱斗 | D 王中王 | E 指定兵种 | F 王中王锦标赛 |
|---|---|---|---|---|---|---|
...
单位人口占用房子槽位。AoE3 中 1 个房子 = 100 木 = 10 人口，即**每人口 ≈ 10 资源**。

单位有效成本 = 基础资源成本 + `POP_HOUSE_COST × 人口`

- `POP_HOUSE_COST = 5`，定义在 `lineup.py`（可调）
- 在所有资源平衡（LCM、贪心分配）和超时判胜中生效
- 显示时人口部分以括号标注：`💰总资源 9000（人口 +900）`
- **不影响**"查兵种"信息卡（信息卡显示原始肉木金）
...
- `王中王` 列入 launcher 模式关键词，避免 `斗蛐蛐 王中王 散兵` 被误判为指定兵种名
- `斗蛐蛐 火枪手`（无王中王前缀）仍走指定兵种逻辑
...
- **指令**：`斗蛐蛐 N时代`（N ∈ **2~5**）指定本局时代；可与现有模式/预算组合（如 `斗蛐蛐 火枪 散兵 5时代`）。**所有模式**都接受该参数（§④）。
...
**每局随机规则**：
- **每方各 K/2 条**：单挑/指定兵种 K=2 → 每方 1 条；押注 K=4 → 每方 2 条
...
- ✅ **新增 `GameMode(id="blacklist")`**：在 `aoe3_battle/game.py` 注册，公开模式名为 `乱斗`。
...
- ✅ **对阵**：红蓝各 1 兵种 + LCM（同指定兵种对决），**不是**单挑 1v1
