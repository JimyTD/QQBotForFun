...
开发实现与验收：
- 局部绕行使用膨胀障碍轮廓上的可见性搜索，保留错列障碍之间的通道；默认最多采样 24 个障碍、展开 96 个搜索节点。
...
- 椭圆单位正常沿移动方向逐步转向；受阻时可保持当前朝向先平移出空间，也可原地小幅转向。所有候选动作均检查完整位姿扫掠，不能仅凭起点和终点不重叠就放行。默认占位转向上限为每秒 2π 弧度，是本模型运动求解参数，不宣称读取了原版转向数据；它不改变攻击抬手或射速。
