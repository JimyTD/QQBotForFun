...
> **2026-09-23 更新（当前正式口径）**：普通对局不再逐窗口播报，也不再发送开战提示。
> 当前流程为：
>
> 1. 开局发送一张通用开局图（国战复用国战开局图，其余模式使用无国家字段的通用开局图）。
> 2. 押注说明包含在开局图中；实际押注仍逐条简短确认。
> 3. 战斗过程静默；最终依次发送战场回放视频、精简文字战报、押注结算（无押注不发结算）。
> 4. 文字战报保留胜负、时长、剩余兵力/HP、战损资源、各兵种击杀与伤害，不再输出 `xxx 全灭`。
> 5. 锦标赛每场仍输出一次文字战报；只有决赛录制并发送回放视频，其余轮次不生成视频。
>
> 以下 §8.2–§8.5 保留为历史广播模型，其逐窗口、全灭和过程话术规则当前不再由正式群局调用。

### 3.1 场地模型

- **二维矩形场地**：x 轴是双方接近方向，y 轴是横向展开方向
- 单位拥有圆形碰撞体积，半径为 `unit_radius`
