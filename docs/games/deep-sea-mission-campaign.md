...
- **自荐包揽兜底**：M14/15/16/26 全员都「过」时按官方由**队长必须接下**（不存在「退回轮流选」）；M26 缺 1 个名额时队长补位，缺 2 个或队长本身就是唯一自愿者时改为队长一人包揽全部任务（不强迫其他玩家）。M6 是「全队共同决定一人承担」，无人自愿继续表态。
- **进度持久化**：`group_config` key `deep_sea_mission.campaign.level`，通关 +1、失败不推进、M32 后进 Epilogue（难度 18 起步 +1）。
- **传牌**：`MOD_DISTRESS` 无固定关卡，实现为选任务阶段可触发（`@我 求救` → 每人 `@我 传 X` 传给**下家**，禁传潜艇）。**当前冻结**：机器人不宣传该入口（官方 ⚓ 关卡清单未确认），拿到清单后按关开放。
