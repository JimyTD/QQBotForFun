...
| 深海任务声呐入口 | 墩间出现一条共享提示，输入 `声呐 蓝4 最高`，**按牌面反查归属** | 任一玩家随时 `@我 声呐 …`，归属 = 发话人 | 机制差异（时机/写法/次数/文案取自 `game.py` 共用实现，逐字一致） |
| 静夜标记私聊展示 | 终端用 `[私聊 3号 小刚]` 前缀标出私密消息（一个人扮所有座位，必须一眼看出信息边界） | 走 `session.whisper`，只有当事人看得到 | 机制差异（文案取自 `views.py` / `syntax.py` 同一份，逐字一致） |
| 静夜标记撤回 | 不打印撤回日志（CLI 是滚动日志，撤回只会添乱） | 看板/提问/玩家指令会真的撤回 | 机制差异（消息生命周期由 `game.py` 同一份实现驱动） |
| 静夜标记奖励 | 不发奖，避免 CLI 测试数据污染真实金币/积分 | 胜负双方都发奖 | 机制差异（仅经济副作用，不影响任何玩家可见文案） |
...
- **静夜标记中断语义**：`结束`（CLI Ctrl+C/管道结束也算）= 终止整局且**任何人可用**；
  `退出` = 只让自己出局（认输，遗物公开，别人继续）。两端逐字一致
- **深海任务声呐**：时机判定 / 写法解析 / 次数（含 rapture 共享额度、silence 禁用）/ 全部提示文案都来自 `game.py` 同一份实现，CLI 与 Bot 逐字一致
