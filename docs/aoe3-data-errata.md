...
## 二、射击军（strelet）cost 异常

| id | 中文名 | 当前 cost | 预估真实 cost | HP | 问题 |
|---|---|---|---|---|---|
| `strelet` | 射击军 | 10 | ~62（47食物+15金币） | 72 | 3000 预算 = 300 个（应为 ~48 个） |

**结论**：cost=10 是数据 bug。射击军确实是游戏中较便宜的兵种，但真实费用约 47F+15G=62，不是 10。
当前未加入黑名单（不影响游戏平衡到需要屏蔽的程度），但数据应修正。
