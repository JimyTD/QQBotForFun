...
BATTLE_BLACKLIST: dict[str, str] = {
    ...
    # —— 2026-09 新版弑君模式国王（hp 2000+、攻击仅 10、不占人口、无训练途径） ——
    # 与既有在池单位 ypdaimyoregicide（hp 2000 / atk 10）同型；2026-09-17 用户决议归入黑名单：
    # 拉进普通对战只会变成"打不死的肉盾"，拖到超时按 HP 比判胜。
    "deregent":             "剧情·摄政王(步)",       # hp 2500 atk 10 flags=NotDeleteable
    "deregenthorse":        "剧情·摄政王(骑)",       # hp 2000 atk 10
    "despchmlord":          "剧情·领主",             # hp 2026 atk 10 RegicideAlertOnDeath
...
# 黑名单乱斗常量（详见设计文档 §2.4）
# =====================================================================
# 兵种数权重：黑名单池规模有限（2026-09-17 为 28 个），3 兵种局展示拥挤、节目效果分散，
# 因此只保留 1 / 2 兵种概率。
